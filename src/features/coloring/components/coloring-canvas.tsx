...
export function ColoringCanvas({ imageSrc, pageId, title, description, category, rootKeyword, relatedPages }: ColoringCanvasProps) {
  ...
  const [pinterestBindDialogOpen, setPinterestBindDialogOpen] = useState(false);
  ...
  // Check Pinterest binding status
  useEffect(() => {
    let mounted = true;
  ...
  const handlePinterestShare = async () => {
    // Check if Pinterest is connected first
    if (!pinterestConnected) {
      setPinterestBindDialogOpen(true);
      return;
    }
  ...
  return (
    <div className="flex flex-col w-full items-start max-w-6xl mx-auto px-4">
      ...
      {/* Pinterest Bind Dialog */}
      <Dialog open={pinterestBindDialogOpen} onOpenChange={setPinterestBindDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Connect Pinterest Account</DialogTitle>
            <DialogDescription>
              Connect your account to share your coloring pages
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setPinterestBindDialogOpen(false)}>Cancel</Button>
            <Button
              onClick={() => {
